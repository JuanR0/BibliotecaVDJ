  ...
  <div class="user-management">
    ...
    <div class="tabla-section">
      ...
      <div v-else class="tabla-wrap">
        <table class="tabla">
          ...
          <tbody>
            <tr
              ...
              :key="user.id"
              :class="{ 'fila-inactiva': !user.esta_activo }"
            >
              <td class="td-id">#{{ user.id }}</td>

              <td>
                <span class="td-codigo">{{ user.codigo_universitario }}</span>
              </td>
              ...
              <td>
                <div class="acciones">
                  ...
                  <button
                    v-if="canEditUser(user)"
                    class="accion-btn accion-reset"
                    title="Resetear contraseña"
                    :disabled="isProcessing"
                    @click="abrirModalReset(user)"
                  >
                    <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  ...
                  <!-- Activar / Desactivar -->
                  <button
                    v-if="user.esta_activo && canEditUser(user)"
                    class="accion-btn accion-deact"
                    title="Desactivar"
                    :disabled="isProcessing"
                    @click="confirmarAccion(user, 'deactivate')"
                  >
                    <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  ...
                  <button
                    ...
                    class="accion-btn accion-act"
                    title="Activar"
                    :disabled="isProcessing"
                    @click="confirmarAccion(user, 'activate')"
                  >
                    <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  ...
                  <button
                    v-if="!user.esta_activo"
                    class="accion-btn accion-delete"
                    title="Eliminar permanentemente"
                    :disabled="isProcessing"
                    @click="confirmarAccion(user, 'delete')"
                  >
                    <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div v-if="filteredUsers.length === 0" class="estado-vacio">
          ...
          <button class="btn-retry" @click="resetFilters">Limpiar filtros</button>
        </div>
      </div>
      ...
      <div v-if="!isLoading && filteredUsers.length > 0" class="paginacion">
        ...
        <div class="pag-controles">
          ...
          <span
            ...
            :key="page"
            class="pag-num"
            :class="{ 'pag-num-active': page === currentPage }"
            @click="goToPage(page)"
          >{{ page }}</span>
        ...
        <div class="modal-body">
          <div class="form-grid">
            <div class="form-field">
              ...
              <input v-model="crearForm.codigo_universitario" type="text"
                class="form-input" placeholder="U20234567" maxlength="20"/>
            </div>

            <div class="form-field">
              <label class="form-label">Nombre Completo <span class="req">*</span></label>
              <input v-model="crearForm.nombre_completo" type="text"
                class="form-input" placeholder="Juan Pérez García" maxlength="255"/>
            </div>

            <div class="form-field">
              <label class="form-label">Contraseña inicial <span class="req">*</span></label>
              <input v-model="crearForm.clave_acceso" type="password"
            ...
            <div class="form-field">
              ...
              <select v-model="crearForm.tipo_usuario_id" class="form-select">
                <option v-for="t in userTypes" :key="t.id" :value="t.id"
                  :disabled="t.id === 4">
                  {{ t.nombre }}
                </option>
              </select>
            ...
            <div class="form-field">
              ...
              <select v-model="crearForm.relacion_institucional_id" class="form-select">
                <option v-for="r in institutionalRelations" :key="r.id" :value="r.id">
                  {{ r.nombre }}
                </option>
              </select>
            </div>
          ...
          <div v-if="modalCrear.error" class="form-error">
            {{ modalCrear.error }}
          </div>
        </div>

        <div class="modal-footer">
          <button class="btn-cancelar" @click="cerrarModalCrear" :disabled="isProcessing">Cancelar</button>
          <button class="btn-confirmar" @click="guardarNuevoUsuario" :disabled="isProcessing">
    ...
    <div v-if="modalEditar.visible" class="modal-overlay" @click.self="cerrarModalEditar">
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="form-grid">
            <div class="form-field">
              ...
              <input :value="editForm.codigo_universitario" type="text"
            ...
            <div class="form-field">
              <label class="form-label">Nombre Completo <span class="req">*</span></label>
              <input v-model="editForm.nombre_completo" type="text"
            ...
            <div class="form-field form-field-full">
              ...
              <div class="estado-toggle">
                <button
                  class="toggle-opt"
                  :class="{ 'toggle-opt-on': editForm.esta_activo }"
                  @click="editForm.esta_activo = true"
                >Activo</button>
                <button
                  class="toggle-opt"
                  :class="{ 'toggle-opt-off': !editForm.esta_activo }"
        ...
        <div class="modal-footer">
          <button class="btn-cancelar" @click="cerrarModalEditar" :disabled="isProcessing">Cancelar</button>
          <button class="btn-confirmar" @click="guardarEdicion" :disabled="isProcessing">
            <span v-if="isProcessing" class="spinner-mini"></span>
    ...
    <div v-if="modalReset.visible" class="modal-overlay" @click.self="cerrarModalReset">
      <div class="modal modal-sm">
        ...
        <div class="modal-footer">
          <button class="btn-cancelar" @click="cerrarModalReset" :disabled="isProcessing">Cancelar</button>
          <button class="btn-reset-confirm" @click="ejecutarReset" :disabled="isProcessing">
            <span v-if="isProcessing" class="spinner-mini"></span>
    ...
    <div v-if="modalConfirm.visible" class="modal-overlay" @click.self="cerrarModalConfirm">
      <div class="modal modal-sm">
        ...
        <div class="modal-footer">
          <button class="btn-cancelar" @click="cerrarModalConfirm" :disabled="isProcessing">Cancelar</button>
          <button
            :class="modalConfirm.accion === 'delete' ? 'btn-danger' : 'btn-confirmar'"
...
const router    = useAuthStore()
const authStore = useAuthStore()

// Redirigir si no es tipo 4
...
const modalConfirm = ref({
  ...
  textoBtn: '', accion: '', usuario: null
})

// ── Formularios ────────────────────────────────────────────────────────────
const crearForm = ref({
  codigo_universitario: '', nombre_completo: '',
  clave_acceso: '', tipo_usuario_id: 1, relacion_institucional_id: 1
})

const editForm = ref({
  id: null, codigo_universitario: '', nombre_completo: '',
  tipo_usuario_id: 1, relacion_institucional_id: 1, esta_activo: true
})
...
const filteredUsers = computed(() => {
  ...
  if (!showInactiveUsers.value) {
    result = result.filter(u => u.esta_activo)
  }

  if (filters.value.tipo_usuario_id) {
    result = result.filter(u => u.tipo_usuario_id === parseInt(filters.value.tipo_usuario_id))
  }
...
const cerrarModalCrear = () => {
  if (!isProcessing.value) modalCrear.value.visible = false
}
...
const guardarNuevoUsuario = async () => {
  ...
  if (!codigo_universitario || !nombre_completo || !clave_acceso) {
    modalCrear.value.error = 'Completa todos los campos obligatorios'
    return
  }
  if (clave_acceso.length < 6) {
    modalCrear.value.error = 'La contraseña debe tener al menos 6 caracteres'
    return
  }

  isProcessing.value = true
  try {
    const nuevo = await userService.createUser({
      codigo_universitario, nombre_completo, clave_acceso,
      tipo_usuario_id, relacion_institucional_id
    })
...
const cerrarModalEditar = () => {
  if (!isProcessing.value) modalEditar.value.visible = false
}
...
const guardarEdicion = async () => {
  ...
  isProcessing.value = true
  try {
    const actualizado = await userService.updateUser(editForm.value.id, {
    ...
    if (idx !== -1) Object.assign(users.value[idx], actualizado)
    cerrarModalEditar()
...
// ── Modal reset password ───────────────────────────────────────────────────
const abrirModalReset = (user) => {
  modalReset.value = { visible: true, usuario: user }
}

const cerrarModalReset = () => {
  if (!isProcessing.value) modalReset.value.visible = false
}
...
const getRelacionNombre = (id) => {
  const rel = institutionalRelations.value.find(r => r.id === id)
  return rel?.nombre || '—'
}
...
.header-titulo {
  ...
  font-size: 1.6rem; font-weight: 700; color: #fff; margin-bottom: .3rem;
}
.header-sub {
...
.btn-toggle-inactivos:hover,
.btn-toggle-on { background: rgba(255,255,255,.18); color: #fff; }
...
.btn-crear {
  ...
  box-shadow: 0 4px 14px rgba(244,197,66,.3);
  transition: all .2s;
}
...
.filter-select {
  ...
  font-size: .82rem; color: #3d5a3d; outline: none; cursor: pointer;
}
...
.btn-reset {
  ...
  color: #5a7a5a; cursor: pointer; white-space: nowrap;
  transition: all .18s;
}
...
.tabla thead th {
  ...
  color: rgba(255,255,255,.88); font-size: .72rem;
  font-weight: 700; text-transform: uppercase; letter-spacing: .06em;
}

.tabla tbody tr {
  border-bottom: 1px solid #f1f5f0; transition: background .15s;
}
...
.accion-btn {
  ...
  padding: .3rem .6rem; border: 1.5px solid;
...
.accion-edit   { background: #f0f7f2; color: var(--green-mid);  border-color: #b8ddc8; }
.accion-reset  { background: var(--gold-pale); color: var(--gold-mid); border-color: #fde68a; }
.accion-deact  { background: #fff8e1; color: #92400e; border-color: #fcd34d; padding: .3rem .5rem; }
.accion-act    { background: var(--green-pale); color: var(--green-dark); border-color: #b8ddc8; padding: .3rem .5rem; }
.accion-delete { background: #fff0f0; color: #b91c1c; border-color: #fca5a5; padding: .3rem .5rem; }
...
.estado-loading p, .estado-error p { color: #5a7a5a; font-size: .875rem; }
...
.btn-retry {
  padding: .45rem 1.1rem; background: var(--green-mid); color: #fff;
  border: none; border-radius: 8px; font-size: .82rem; font-weight: 700;
  cursor: pointer; transition: background .2s;
}
.btn-retry:hover { background: #111; }
...
.pag-num {
  ...
  font-size: .8rem; font-weight: 500; color: var(--green-mid); transition: all .15s;
}
...
.pag-select {
  ...
  border-radius: 7px; font-size: .8rem; color: var(--green-dark);
...
.modal-header h3 {
  font-family: 'Playfair Display', serif;
  font-size: 1.05rem; font-weight: 700; color: var(--green-dark); margin: 0;
}
.modal-close {
  background: none; border: none; font-size: 1.3rem;
  color: #9ab5a0; cursor: pointer; padding: 0; line-height: 1;
...
.form-input, .form-select {
  ...
  border-radius: 9px; font-family: 'DM Sans', sans-serif;
  font-size: .875rem; color: #1a2e1a; outline: none;
...
.form-hint {
  font-size: .7rem; color: #9ab5a0; margin: 0;
}
.form-error {
  margin-top: .75rem; padding: .6rem .875rem;
  background: #fff3f3; border: 1px solid #f5c0c0;
  border-radius: 9px; color: #d62828; font-size: .82rem; font-weight: 600;
}
...
.reset-info {
  ...
  padding: .875rem; background: var(--cream);
...
.reset-aviso {
  ...
  padding: .75rem; background: var(--gold-pale);
  border: 1px solid #fde68a; border-radius: 9px;
  font-size: .82rem; color: #7a5200;
}
...
.confirm-user {
  ...
  padding: .75rem; background: var(--cream);
  border-radius: 9px; border-left: 3px solid var(--green-light);
  font-size: .82rem;
}
...
.btn-cancelar {
  ...
  font-weight: 600; color: #5a7a5a; cursor: pointer; transition: all .18s;
}
.btn-cancelar:hover    { background: #f8fafc; }
.btn-cancelar:disabled { opacity: .5; cursor: not-allowed; }

.btn-confirmar {
  padding: .5rem 1.1rem; background: var(--green-mid); color: #fff;
...
.btn-reset-confirm {
  padding: .5rem 1.1rem; background: var(--gold-mid); color: #fff;
  border: none; border-radius: 8px; font-family: 'DM Sans', sans-serif;
  font-size: .82rem; font-weight: 700; cursor: pointer;
  display: inline-flex; align-items: center; gap: .4rem;
  transition: background .2s;
}
.btn-reset-confirm:hover:not(:disabled) { background: #111; }
.btn-reset-confirm:disabled { opacity: .5; cursor: not-allowed; }

.btn-danger {
  padding: .5rem 1.1rem; background: #d62828; color: #fff;
  border: none; border-radius: 8px; font-family: 'DM Sans', sans-serif;
  font-size: .82rem; font-weight: 700; cursor: pointer;
  display: inline-flex; align-items: center; gap: .4rem;
  transition: background .2s;
}
.btn-danger:hover:not(:disabled) { background: #111; }
