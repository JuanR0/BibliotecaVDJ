  ...
  <div class="gestion-prestamos">
    ...
    <header class="page-header">
      <div class="header-inner">
        <div class="header-texto">
          ...
          <h1 class="header-titulo">Préstamos de Cubículos</h1>
          <p class="header-sub">Gestiona el uso de áreas de estudio del CID</p>
        </div>
    ...
    <div class="toolbar">
      ...
      <div class="search-wrap">
        ...
        <input v-model="busqueda" type="text" class="search-input" placeholder="Buscar por estudiante o área..."/>
    ...
    <div class="tabla-section">
      ...
      <div v-else-if="filtrados.length === 0" class="estado-v">
        ...
        <p class="vacio-titulo">{{ busqueda ? 'Sin resultados' : activeTab === 'historial' ? 'Sin historial' : 'No hay cubículos prestados' }}</p>
      </div>
        ...
        <table class="tabla">
          ...
          <tbody>
            <tr v-for="p in paginados" :key="p.id" :class="getRowClass(p)">
              ...
              <td>
                <div class="td-titulo">{{ p.area_nombre }}</div>
                <div class="td-sub" v-if="p.area_capacidad">Capacidad: {{ p.area_capacidad }} personas</div>
              </td>
              <td>
              ...
              <td>
                <div class="td-fecha">{{ activeTab === 'historial' ? fmtHora(p.fecha_devolucion_real) : fmtHora(p.fecha_devolucion_esperada) }}</div>
                <div v-if="activeTab !== 'historial'" class="tiempo-info">{{ formatTiempo(p.fecha_devolucion_esperada) }}</div>
                <div v-else-if="p.horas_excedidas > 0" class="tiempo-info excedido-hist">+{{ p.horas_excedidas }}h excedido</div>
              </td>
              <td>
    ...
    <div v-if="modalCrear.visible" class="modal-overlay" @click.self="cerrarModalCrear">
      <div class="modal">
        <div class="modal-header">
          <h3>Registrar Préstamo de Cubículo</h3>
        ...
        <div class="modal-body">
          ...
          <div class="form-field">
            <label class="form-label">Área / Cubículo <span class="req">*</span></label>
            <div v-if="!crearForm.areaSeleccionada">
              <input v-model="crearForm.busquedaArea" type="text" class="form-input" placeholder="Buscar área disponible..." @input="buscarAreas"/>
              <div v-if="buscandoAreas" class="sug-cargando">Buscando...</div>
              <div v-else-if="areasSug.length" class="sugerencias">
                <div v-for="a in areasSug" :key="a.id" class="sug-item" @click="selArea(a)">
                  <div class="sug-titulo">{{ a.nombre }}</div>
                  <div class="sug-sub">Capacidad: {{ a.capacidad }} · {{ a.tipo_area_nombre || a.estado_nombre }}</div>
                </div>
              </div>
              <div v-else-if="crearForm.busquedaArea.length >= 1" class="sug-vacio">Sin áreas disponibles</div>
            </div>
            <div v-else class="seleccionado">
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" style="flex-shrink:0;color:var(--green-mid)"><path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
              <div class="sel-info">
                <div class="sel-titulo">{{ crearForm.areaSeleccionada.nombre }}</div>
                <div class="sel-sub">Capacidad: {{ crearForm.areaSeleccionada.capacidad }}</div>
              </div>
              <button class="sel-quitar" @click="crearForm.areaSeleccionada = null; crearForm.busquedaArea = ''">×</button>
            </div>
          </div>
          ...
          <div class="form-field">
            ...
            <input type="datetime-local" v-model="crearForm.fecha_devolucion_esperada" class="form-input" :min="ahora"/>
            <p class="form-hint">El cubículo se presta por horas</p>
          </div>

          <div class="form-field">
            <label class="form-label">Observaciones</label>
            <textarea v-model="crearForm.observaciones" class="form-input form-textarea" rows="2" placeholder="Número de personas, notas..."></textarea>
          </div>
      ...
      <div class="modal modal-sm">
        ...
        <div class="modal-body">
          <div class="resumen-prestamo">
            <div class="rp-row"><span class="rp-lbl">Área</span><span class="rp-val">{{ modalDevolver.prestamo?.area_nombre }}</span></div>
            ...
            <div class="rp-row"><span class="rp-lbl">Debía devolver</span><span class="rp-val">{{ fmtHora(modalDevolver.prestamo?.fecha_devolucion_esperada) }}</span></div>
          </div>

          <div v-if="modalDevolver.prestamo" class="tiempo-actual" :class="'tiempo-actual-' + calcularEstadoTiempo(modalDevolver.prestamo.fecha_devolucion_esperada)">
            <strong>{{ getTiempoBadgeText(modalDevolver.prestamo) }}</strong> — {{ formatTiempo(modalDevolver.prestamo.fecha_devolucion_esperada) }}
          </div>

          <div class="form-field">
            <label class="form-label">Observaciones</label>
            <textarea v-model="modalDevolver.observaciones" class="form-input form-textarea" rows="2" placeholder="Condición del área al devolver..."></textarea>
          </div>
...
import { prestamoAreaService, calcularEstadoTiempo, formatTiempo } from '@/services/PrestamoRecursos'
...
const crearForm = ref({ busquedaArea: '', areaSeleccionada: null, busquedaUsuario: '', usuarioSeleccionado: null, fecha_devolucion_esperada: '', observaciones: '' })
const areasSug        = ref([])
const usuariosSug     = ref([])
const buscandoAreas   = ref(false)
...
const tabs = computed(() => {
  ...
  const enAlerta  = vigentes.filter(p => ['advertencia','peligro','excedido'].includes(calcularEstadoTiempo(p.fecha_devolucion_esperada)))
  return [
    { id: 'vigentes',  label: 'Vigentes', count: vigentes.length, badgeClass: 'badge-green' },
    { id: 'alertas',   label: 'Alertas',  count: enAlerta.length, badgeClass: 'badge-red' },
...
const filtrados = computed(() => {
  ...
  if (activeTab.value === 'vigentes') r = r.filter(p => p.estado_prestamo_id === 1 && !['peligro','excedido'].includes(calcularEstadoTiempo(p.fecha_devolucion_esperada)))
  if (activeTab.value === 'alertas')  r = r.filter(p => p.estado_prestamo_id === 1 && ['advertencia','peligro','excedido'].includes(calcularEstadoTiempo(p.fecha_devolucion_esperada)))
...
const getTiempoBadgeClass = (p) => {
  ...
  const est = calcularEstadoTiempo(p.fecha_devolucion_esperada)
...
const cargar = async () => {
  ...
  try {
    prestamos.value = await (activeTab.value === 'historial'
      ? prestamoAreaService.getHistorial()
      : prestamoAreaService.getVigentes())
  } catch (e) {
...
const buscarAreas = () => {
  clearTimeout(timerA); areasSug.value = []
  const q = crearForm.value.busquedaArea.trim()
  buscandoAreas.value = true
  timerA = setTimeout(async () => {
    try {
      const r = await api.get('/api/areas/', { params: { estado_id: 1, es_prestable: true, por_pagina: 20 } })
      const todas = r.data?.areas ?? []
      areasSug.value = q.length === 0
        ? todas
        : todas.filter(a => a.nombre?.toLowerCase().includes(q.toLowerCase()))
    } catch { areasSug.value = [] }
    finally { buscandoAreas.value = false }
  }, 250)
}
...
const selArea    = (a) => { crearForm.value.areaSeleccionada = a;   crearForm.value.busquedaArea = '';   areasSug.value = [] }
const selUsuario = (u) => { crearForm.value.usuarioSeleccionado = u; crearForm.value.busquedaUsuario = ''; usuariosSug.value = [] }

// ── Modal crear ────────────────────────────────────────────────────────────
const abrirModalCrear = () => {
  crearForm.value = { busquedaArea: '', areaSeleccionada: null, busquedaUsuario: '', usuarioSeleccionado: null, fecha_devolucion_esperada: '', observaciones: '' }
  areasSug.value = []; usuariosSug.value = []
...
const guardarPrestamo = async () => {
  if (!crearForm.value.areaSeleccionada)     { modalCrear.value.error = 'Selecciona un área'; return }
  if (!crearForm.value.usuarioSeleccionado)  { modalCrear.value.error = 'Selecciona un estudiante'; return }
  if (!crearForm.value.fecha_devolucion_esperada) { modalCrear.value.error = 'Indica la hora de devolución'; return }
  procesando.value = true
  try {
    const nuevo = await prestamoAreaService.crearPrestamo({
      area_id:                   crearForm.value.areaSeleccionada.id,
      usuario_prestado_id:       crearForm.value.usuarioSeleccionado.id,
      fecha_devolucion_esperada: crearForm.value.fecha_devolucion_esperada,
    ...
    mostrarToast(`Préstamo de "${crearForm.value.areaSeleccionada.nombre}" registrado`)
  } catch (e) {
...
const confirmarDevolucion = async () => {
  ...
  procesando.value = true
  try {
    await prestamoAreaService.registrarDevolucion(p.id, modalDevolver.value.observaciones || null)
    prestamos.value = prestamos.value.filter(x => x.id !== p.id)
    modalDevolver.value.visible = false
    mostrarToast(`Área "${p.area_nombre}" devuelta correctamente`)
  } catch (e) {
...
.excedido-hist { color:#dc2626; font-weight:600; }
